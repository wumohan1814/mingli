...
// ===== B1 全站 UI 文案常量（REQ-037）=====
// 来源：docs/文案交付/B1_ui_copy.json（去掉 _meta，8 组全量内联）。
// 集中维护：落地页 / 品牌标语 / 免责声明 / Hub 标题等从此引用。
// buttons / tips / empty / error 为本轮（C5）全站统一文案源；各模块按钮/提示/错误态均从此引用，不再散落硬编码。
const UI_COPY = {
  brand: {
    name: '太初',
    tagline: '识己于星，安己于时',
  ...
  landing: {
    ...
    disclaimer_short: '太初所载皆为文化趋势参考，仅供自我觉察，不构成任何决策建议。'
  },
  module_hub: {
    guoxue_title: '国学预测',
  ...
  compliance: {
    disclaimer_full: '太初所提供的所有内容，均基于传统命理与占星文化的趋势性参考，用于自我觉察与思路梳理，不构成医疗、投资、司法或任何具有约束力的决策建议。命运向来开放，真正的结果，来自你清醒的选择与持续的行动。',
  ...
  forceGate: {
    tip: '你还没有任何命理档案。请先完成下方建档；建好至少 1 份档案后，即可解锁全部功能（含各占卜 / 测试模块、功能设置与太初先生）。'
  },
  // REQ-082 / REQ-083：星座「恋爱 / 朋友 / 上下级」关系匹配入口与弹窗文案（静态轻内容、免费；
  // 分值 + 说明文字从 window.TC_PAIRS（/data/pairs.js，T1–T3 文案交付）查表渲染）
  sunRel: {
    cardTitle: '太阳星座 · 关系匹配',
  ...
  // 六合=5、三合=4.5、相冲=2.0、相害=2.5、三刑=2.0、其余=3.0；说明文字从 window.TC_PAIRS
...
const AUTH_REQUIRED_PAGES = ['cases','onboarding','waiting','calibration','predict','revise','topic','archive','credits','zodiac','divination','tarot','lenormand','astrology','mbti','nine-pick','agent'];
...
// banner_dropdown,share_taichu_ui,bg_enabled,card_images,agent_enabled}}}；PUT 部分字段 → 全量 settings。
// 前端默认值见 SETTINGS_DEFAULTS；未登录或接口失败一律用前端默认值（登录后由 App 拉取覆盖）。
// 运行期镜像 TC_SETTINGS 供各页面/钩子读取（模块级对象原地 Object.assign，页挂载即读到最新值），
// 另暴露 camelCase 只读钩子（animEnabled / defaultMode / bannerDropdown / shareTaichuUi /
...
const SETTINGS_DEFAULTS = {
  ...
  share_taichu_ui: true,
  bg_enabled: true,
  card_images: true,
  agent_enabled: true
};
const SETTINGS_KEYS = Object.keys(SETTINGS_DEFAULTS);
const TC_SETTINGS = Object.assign({}, SETTINGS_DEFAULTS);
(function installTcAliases() {
  const map = {
    anim_enabled: 'animEnabled',
    default_mode: 'defaultMode',
    banner_dropdown: 'bannerDropdown',
    share_taichu_ui: 'shareTaichuUi',
    bg_enabled: 'bgEnabled',
    card_images: 'cardImages',
    agent_enabled: 'agentEnabled'
  };
  Object.keys(map).forEach(function (k) {
    Object.defineProperty(TC_SETTINGS, map[k], {
      enumerable: false,
      configurable: true,
      get: function () { return TC_SETTINGS[k]; }
    });
  });
})();
// 服务端原始值 → 前端规范化设置对象（未知/缺失字段回退默认值；default_mode 白名单校验）
function normalizeTcSettings(raw) {
...
function tcAskNextDefault(keepCurrent) {
  const dm = TC_SETTINGS.default_mode;
...
function switchShareGuestSession(entering) {
  ...
  let saved = null;
  try {
    saved = localStorage.getItem('taichu_token');
  } catch (e) {
    saved = null;
  }
  if (saved) {
    token = saved;
    try {
      refreshToken = localStorage.getItem('taichu_refresh_token') || '';
    } catch (e) {
      refreshToken = '';
    }
  }
...
// ≥1 档案 → 放行。强制期全部功能入口（含 landing、各 HUB、功能设置、太初先生等不依赖档案的
...
// 充值链接：严禁前端硬编码；由后端/部署时注入 window.TAICHU_RECHARGE_URL。
// 未注入时回退占位，保证按钮可点击不报错（待后端下发真实小鹅通链接）。
const RECHARGE_FALLBACK = '/recharge';
function getRechargeUrl() {
  return typeof window !== 'undefined' && window.TAICHU_RECHARGE_URL || RECHARGE_FALLBACK;
}

// —— 充值（金数据表单）+ 到账轮询 ——
// 后端 POST /api/credits/charge-code（需 JWT）→ {code:0, data:{code,url}}，url 即金数据充值表单；
// GET /api/credits/balance → {code:0, data:{balance,...}}，用于轮询到账（到账并非实时，通常 1–5 分钟）。
const RECHARGE_POLL_MS = 4000; // 每 4 秒查一次余额（3–5 秒区间）
const RECHARGE_MAX_WAIT_MS = 10 * 60 * 1000; // 等待上限：10 分钟
const RECHARGE_PENDING_KEY = 'taichu_pending_recharge';
let rechargePollTimer = null;
let rechargePollStartAt = 0;
let rechargePreBalance = null;

// 读当前余额（兼容双信封：data.balance 或顶层 balance）
...
function stopRechargePoll() {
  ...
  try {
    sessionStorage.removeItem(RECHARGE_PENDING_KEY);
  } catch (e) {/* 忽略 */}
}

// 到账轮询：每 4 秒 GET /api/credits/balance；余额 > 充值前 → 到账提示；超过 10 分钟 → 放弃提示
function startRechargePoll(preBalance, startAt) {
  stopRechargePoll();
  rechargePreBalance = preBalance == null ? null : Number(preBalance);
  rechargePollStartAt = startAt || Date.now();
  const tick = async () => {
    if (Date.now() - rechargePollStartAt > RECHARGE_MAX_WAIT_MS) {
      stopRechargePoll();
      toast('充值处理中，预计 1–5 分钟到账');
      return;
    }
    if (rechargePreBalance == null) return; // 未能取得充值前余额则只计时、不做增减判断
    try {
      const b = await fetchCreditBalance();
      if (b != null && b > rechargePreBalance) {
        stopRechargePoll();
        toast('充值成功，余额已到账');
      }
    } catch (e) {/* 单次查询失败静默，下轮重试 */}
  };
  tick();
  rechargePollTimer = setInterval(tick, RECHARGE_POLL_MS);
}

// 金数据表单整页跳转、用户返回本页（刷新/重开）后，凭 sessionStorage 标记续查到账
function resumePendingRechargePoll() {
  let marker = null;
  try {
    marker = JSON.parse(sessionStorage.getItem(RECHARGE_PENDING_KEY) || 'null');
  } catch (e) {/* 忽略 */}
  if (!marker) return;
  if (marker.at && Date.now() - marker.at > RECHARGE_MAX_WAIT_MS) {
    stopRechargePoll();
    toast('充值处理中，预计 1–5 分钟到账');
    return;
  }
  if (marker.pre != null) startRechargePoll(marker.pre, marker.at);
}

// 充值入口统一动作：POST /api/credits/charge-code 取 code+url → 记录充值前余额 → 打开金数据表单 → 轮询到账
async function startRecharge() {
  // 1) 记录充值前余额（失败不阻断充值流程）
  let preBalance = null;
  try {
    preBalance = await fetchCreditBalance();
  } catch (e) {/* 忽略 */}
  // 2) 向后端换取本次充值 code 与金数据表单 url（双信封：code:0 时 data 内取值）
  let payload = null;
  try {
    const res = await api('/credits/charge-code', {
      method: 'POST'
    });
    payload = res && res.data || res || {};
  } catch (e) {
    toast(e && e.message ? e.message : '充值服务暂不可用，请稍后重试');
    return; // 失败态：仅提示，保持当前页面
  }
  const code = payload && payload.code != null ? payload.code : '';
  const url = payload && payload.url ? payload.url : '';
  if (!url) {
    toast('未获取到充值链接，请稍后重试');
    return;
  }
  // 3) 埋点
  track('credit_recharge_start', {
    code: String(code)
  });
  // 4) 记录「待确认充值」（sessionStorage：整页跳转后返回时据此续查，且 10 分钟窗口跨跳转保留）
  try {
    sessionStorage.setItem(RECHARGE_PENDING_KEY, JSON.stringify({
      pre: preBalance,
      at: Date.now()
    }));
  } catch (e) {/* 忽略 */}
  // 5) 优先新标签打开表单（本页存活可持续轮询）；被拦截则同标签跳转，返回后由续查接管
  const win = window.open(url, '_blank');
  if (win) {
    try {
      win.opener = null;
    } catch (e) {/* 忽略 */}
    startRechargePoll(preBalance, Date.now());
  } else {
...
// 断前尘九法（轨道动画节点）
const DQC_METHODS = ['八字', '紫微斗数', '六爻', '奇门遁甲', '大六壬', '太乙神数', '七政四余', '铁板神数', '果老星宗'];
...
function _sessionId() {
  let s = localStorage.getItem('taichu_session_id');
  if (!s) {
    s = window.crypto && crypto.randomUUID ? crypto.randomUUID() : 's' + Date.now();
    localStorage.setItem('taichu_session_id', s);
  }
  return s;
}
...
async function api(path, options = {}) {
  ...
  const parseError = e => {
    ...
    let msg = '请求失败';
    const d = e && e.detail;
    if (typeof d === 'string' && d) msg = d;else if (Array.isArray(d) && d.length) msg = d.map(x => x && x.msg || JSON.stringify(x)).join('；');else if (d) msg = JSON.stringify(d);
    return msg;
  };
  ...
  if (r.status === 401 && refreshToken && !options.skip401) {
    try {
      ...
      if (rr.ok) {
        ...
        const newToken = rd.access_token || rd.data && rd.data.access_token;
        const newRefresh = rd.refresh_token || rd.data && rd.data.refresh_token;
        if (newToken) {
          token = newToken;
  ...
  if (r.status === 401 && !options.skip401) {
    ...
    localStorage.removeItem('taichu_refresh_token');
  ...
  const rawDetail = errBody && errBody.detail;
  let errMsg;
  if (Array.isArray(rawDetail)) errMsg = rawDetail.map(x => x && x.msg || JSON.stringify(x)).join('；');else if (typeof rawDetail === 'string' && rawDetail) errMsg = rawDetail;else if (errBody && typeof errBody.message === 'string' && errBody.message) errMsg = errBody.message;else errMsg = '请求失败（HTTP ' + r.status + '）';
  const err = new Error(errMsg);
  err.code = errBody && typeof errBody.code === 'number' ? errBody.code : null;
  err.status = r.status;
  err.payload = errBody;
  // 统一拦截「余额不足」错误码，弹充值引导弹窗（由 App 注册 handler 渲染）
...
function DisclaimerFooter() {
  return /*#__PURE__*/React.createElement("div", {
    ...
    name: "spark",
    size: 14
  })), UI_COPY.compliance.disclaimer_full, /*#__PURE__*/React.createElement("div", {
    className: "legal-links"
  }, /*#__PURE__*/React.createElement("a", {
    href: "/legal/用户协议.html",
    target: "_blank"
  }, "用户协议"), /*#__PURE__*/React.createElement("span", {
    className: "legal-sep"
  }, " · "), /*#__PURE__*/React.createElement("a", {
    href: "/legal/隐私政策.html",
    target: "_blank"
  }, "隐私政策")));
}
...
function SkinSwitcher({
  ...
  return /*#__PURE__*/React.createElement("nav", {
    ...
    disabled: !it.enabled,
    onClick: () => it.enabled ? onPick(it.id) : toast('西式占星 / 塔罗暂未开放')
  }, it.name)));
}
...
function LandingPage({
  ...
  const MODS = {
    guoxue: {
      icon: 'nine',
      title: UI_COPY.module_hub.guoxue_title,
      subtitle: UI_COPY.module_hub.guoxue_desc,
      intro: UI_COPY.landing.value_1_desc,
      cta: UI_COPY.module_hub.enter_guoxue,
      to: 'guoxue-hub',
      /* REQ-084：Q 版动画（素材：美术素材表-第三轮 §0.4 主图，静态单图 + CSS keyframes） */
      mascot: { cls: 'guoxue', src: '/art/module-mascot/guoxue-mascot.webp', alt: 'Q 版人物打坐' }
    },
    xishi: {
      icon: 'orbit',
      title: UI_COPY.module_hub.western_title,
      subtitle: UI_COPY.module_hub.western_desc,
      intro: '从本命星盘、行运推演，到塔罗牌阵与雷诺曼小牌——AI 以符号与意象为语，陪你梳理情绪、看清关系与当下的每一个抉择。',
      cta: UI_COPY.module_hub.enter_western,
      to: 'xishi-hub',
      /* REQ-084：Q 版动画（素材：美术素材表-第三轮 §0.4 主图，静态单图 + CSS keyframes） */
      mascot: { cls: 'xishi', src: '/art/module-mascot/xishi-mascot.webp', alt: 'Q 版人物仰望星空' }
    },
    mbti: {
      icon: 'spark',
      title: UI_COPY.module_hub.mbti_title,
      subtitle: UI_COPY.module_hub.mbti_desc,
      intro: '通过 16 型人格框架，梳理你的能量来源、信息偏好与决策方式。逐题作答，AI 将结合你的选择给出专属类型画像与解读。',
      cta: UI_COPY.module_hub.enter_mbti,
      to: 'mbti-hub',
      /* REQ-084：Q 版动画（素材：美术素材表-第三轮 §0.4 主图，静态单图 + CSS keyframes） */
      mascot: { cls: 'mbti', src: '/art/module-mascot/mbti-mascot.webp', alt: 'Q 版人物躺摇椅' }
    }
  };
  ...
  }, c.intro), /* REQ-084：模块 Q 版循环动画（静态单图 + CSS keyframes）—— 整块可点击进入
      太初先生会话（与 banner「对坐谈心」同一 navigate('agent') 入口，REQ-076）；
      REQ-116：agent_enabled 关 → 不渲染，与「对坐谈心」按钮同一开关源 */
  agentEnabled && c.mascot && /*#__PURE__*/React.createElement("button", {
    className: "landing-mascot anim-mascot " + c.mascot.cls,
    type: "button",
    title: "找太初先生聊聊",
    "aria-label": "找太初先生聊聊",
    ...
    className: "lm-cap"
  }, "对坐谈心")), /*#__PURE__*/React.createElement("button", {
...
function CreditBalance({
  ...
  return /*#__PURE__*/React.createElement("span", {
    ...
    tabIndex: 0,
    onClick: onClick,
    title: neg ? '余额不足' : '查看余额明细'
  }, /*#__PURE__*/React.createElement(Icon, {
    name: "spark",
    size: 14
  }), /*#__PURE__*/React.createElement("span", {
    className: "pill-bal"
  }, "余额 ¥", balance == null ? '—' : (Number(balance) / CREDIT_YUAN_RATE).toFixed(2))));
}
...
function BannerBalance({
  ...
  const tagProps = {
    ...
    title: '余额 = 存储单位 ÷ ' + CREDIT_YUAN_RATE + (onClick ? ' · 点击查看余额明细' : '')
  };
  if (onClick) {
...
function payBadge(enabled) {
  ...
  return React.createElement("span", {
    ...
    title: ok ? '按实际用量从余额扣除' : '余额不足，补足后可继续',
    "aria-label": ok ? '¥ 消耗：按实际用量从余额扣除' : '¥ 消耗：余额不足'
  }, "¥ 消耗");
}
...
function MenuBalance({
  ...
  }), "余额 ", /*#__PURE__*/React.createElement("span", {
    className: "mb-val"
  }, "¥", txt));
}
...
function CreditInsufficientModal({
  ...
  }, UI_COPY.error.points_insufficient), /*#__PURE__*/React.createElement("div", {
...
function PairModal({
  ...
  return el('div', {
    ...
    className: 'pair-sub'
  }, '选择两份档案，基于' + meta.dataTxt + '，让 AI 解析两人的关系契合、相处模式与建议。'), el('div', {
    className: 'pair-pay'
  }, '配对解析为付费 LLM 解读：按实际用量扣余额（¥），解读成功后即时扣费（余额不足将自动提示）。'), err ? el('div', {
    className: 'pair-err'
  }, err) : null, busy ? el('div', {
    className: 'pair-busy'
  }, '配对解析生成中（LLM 撰写约需 10–40 秒），请勿重复提交…') : null, res ? el('div', null, el('div', {
  ...
  }, '「' + caseName(aId) + ' × ' + caseName(bId) + '」 · ' + res.relation), el('div', {
    className: 'interp-body',
    style: {
      marginBottom: 12
    }
  }, res.interpretation)) : null, listLoading ? el('div', {
    className: 'pair-busy'
    ...
    className: 'btn btn-outline',
    onClick: load
  }, '重试档案'))) : listEmpty ? el('div', {
    className: 'pair-err'
  }, '暂无可用档案：' + meta.prepTxt + '。') : el(React.Fragment, null, el('div', {
    className: 'pair-sec'
  }, el('div', {
  ...
  }, '（需与档案一不同）'), bId && aId ? el('span', {
    style: {
      fontWeight: 400,
      color: 'var(--skin-accent)',
      fontSize: 11.5
    }
  }, '已选：' + caseName(bId)) : null), el('select', {
    className: 'pair-field',
    value: bId,
    onChange: pick('b')
  }, caseOptions)), el('div', {
    ...
    // 原右对齐（justify-content:flex-end）仅保留给其它复用 .pair-foot 的弹窗
    className: 'pair-foot pair-foot-eq'
  }, el('button', {
    type: 'button',
    className: 'btn btn-outline',
    onClick: onClose
  }, UI_COPY.buttons.close), el('button', {
    type: 'button',
    className: 'btn btn-primary',
...
function AuthPage({
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (!isLogin) {
        ...
        toast(free != null ? '已赠送余额 ¥' + (Number(free) / CREDIT_YUAN_RATE).toFixed(2) : '注册成功，余额已到账');
      }
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "title"
  }, isLogin ? '登录' : '注册'), error && /*#__PURE__*/React.createElement("div", {
    className: "error",
    style: {
      marginBottom: 12
    }
  ...
  }, "图形验证码"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 10,
      alignItems: 'center',
  ...
  }, "看不清？点击图片刷新")), !isLogin && /*#__PURE__*/React.createElement("div", {
    className: "agree-row",
    style: {
      margin: '10px 0 2px'
    }
    ...
    href: "/legal/用户协议.html",
    target: "_blank"
  }, "《用户协议》"), "和", /*#__PURE__*/React.createElement("a", {
    href: "/legal/隐私政策.html",
    target: "_blank"
  }, "《隐私政策》"))), /*#__PURE__*/React.createElement("button", {
    className: "btn btn-primary",
    type: "submit",
    disabled: loading,
    style: {
  ...
  }, loading ? UI_COPY.tips.processing : isLogin ? UI_COPY.buttons.login : UI_COPY.buttons.register)), /*#__PURE__*/React.createElement("p", {
    style: {
      textAlign: 'center',
      marginTop: 16,
      fontSize: 13,
...
function RenameModal({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "modal-title"
  }, "给这份档案起个名字"), /*#__PURE__*/React.createElement("input", {
    className: "input",
    value: value,
    onChange: onChange,
    placeholder: "请输入档案名称",
...
function CaseContactModal({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "modal-title"
  }, title || "修改联系方式"), nameEditable ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("label", {
    className: "label",
    htmlFor: "edit-case-name"
  }, "档案名称"), /*#__PURE__*/React.createElement("input", {
    id: "edit-case-name",
    className: "input",
    type: "text",
    maxLength: 30,
  ...
  }, "档案：", name, "（改名称请用「重命名」）"), /*#__PURE__*/React.createElement("label", {
    className: "label",
    htmlFor: "edit-phone"
...
function ShareFillModal({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "modal-title"
  }, UI_COPY.shareFill.title), url ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 13,
      color: 'var(--text-2)',
      margin: '0 0 10px',
  ...
  }, UI_COPY.shareFill.desc), /*#__PURE__*/React.createElement("input", {
    className: "input",
    readOnly: true,
    value: url,
    title: url,
  ...
  }, UI_COPY.shareFill.hint), /*#__PURE__*/React.createElement("div", {
    className: "flex-row",
    style: {
      marginTop: 16,
      justifyContent: 'center',
    ...
    type: "button",
    onClick: onClose
  }, UI_COPY.buttons.close), /*#__PURE__*/React.createElement("button", {
    className: "btn btn-primary",
    type: "button",
    onClick: onCopy
  }, UI_COPY.buttons.copy_link))) : busy ? /*#__PURE__*/React.createElement("p", {
    style: {
      fontSize: 13,
      color: 'var(--text-2)',
      margin: '6px 0 2px'
    }
  }, UI_COPY.shareFill.generating) : err ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "error",
    style: {
      marginBottom: 10
    }
...
function CasesPage({
  ...
  const handleDelete = async c => {
    ...
    try {
      ...
      setCaseList(list => list.filter(x => x.caseId !== c.caseId));
      toast('档案已删除');
...
const MOD_NAV_GROUPS = [{
  ...
  items: [{
    ...
    // REQ-121：黄历择日（免档案纯国学工具，顶栏下拉与 HUB 入口对齐）
    label: '择日',
    page: 'almanac',
    module: 'guoxue',
    skin: 'guoxue'
  }, {
    // REQ-124：太乙神数（免档案纯国学大势工具，顶栏下拉与 HUB 入口对齐）
    label: '太乙神数',
    page: 'taiyi',
    module: 'guoxue',
    skin: 'guoxue'
  }, {
    // REQ-125：皇极经世（免档案纯国学大势工具，顶栏下拉与 HUB 入口对齐）
    label: '皇极经世',
    page: 'huangji',
    module: 'guoxue',
    skin: 'guoxue'
...
function TopbarModNav({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    "aria-label": "模块导航"
  }, "模块", /*#__PURE__*/React.createElement("span", {
    className: "caret",
    "aria-hidden": "true"
...
function TopbarMenu({
  ...
  }), "功能设置"), /*#__PURE__*/React.createElement("button", {
    className: "menu-item",
    role: "menuitem",
    onClick: () => {
      setOpen(false);
  ...
  }), "档案管理"), /* PWA：「安装到桌面」常驻菜单项（永不消失；点击按状态机分派：a 可安装→prompt / b 已安装→提示 / c 其他→浏览器菜单手动安装指引） */
  /*#__PURE__*/React.createElement("button", {
...
function NinePickPage({
  ...
  if (!list && !errored) {
    ...
    // 档案行：名称 + 九法状态标签（已跑=主题色 / 未跑=灰字）+ 出生 meta + 圆形选中勾
    pickBody = el('div', {
      className: 'case-opts'
    }, list.map(c => {
      const sel = selId != null && String(selId) === String(c.caseId);
      ...
      return el('button', {
        key: c.caseId,
        type: 'button',
        className: 'case-opt' + (sel ? ' sel' : ''),
        title: st.view ? '已跑过九法：勾选后可「查看 / 修改结果」' : st.go ? '未跑九法：勾选后可「跑九法合一开始预测」' : '该档案尚未排盘：请先在档案管理执行排盘',
        onClick: () => setSelId(String(c.caseId))
      }, el('span', {
        className: 'co-main'
      }, el('span', {
        className: 'co-name'
      }, nm), el('span', {
        className: 'co-type' + (st.cls ? ' ' + st.cls : '')
      }, st.label), el('span', {
        className: 'co-meta'
      }, meta)), el('span', {
        className: 'co-check'
...
function NamerModal({
  ...
  }, hd, sub, picker, err ? el('div', {
    className: 'pair-err'
  }, err) : null, busy ? el('div', {
    className: 'pair-busy'
  }, C.submitting) : null, form, result, el('div', {
    className: 'pair-pay'
  }, C.notes), el('div', {
    className: 'pair-foot'
  }, el('button', {
...
function GuoxueHubPage({
  ...
  // REQ-100：配对解析 / 起名 / 八字配对 三个同级功能上移一级 → 与「九法合一 / 生肖流年 / 临时起卦」
  // 同层级成为功能按键，同一排 3 键（九法配对 / 八字配对 / 档案起名）横向摆放，UI 同样式 + I4 图标
  // （fn-pair / fn-bazi / fn-name）；原九法合一选档案页内 .pair-entry 与「起名」行移除（覆盖
  // REQ-093/094 入口布局口径，功能逻辑不变：九法配对/八字配对 → openPairAnalyze 弹 PairModal；
  // 档案起名 → 弹 NamerModal，未预选档案时弹窗内选档案）。本 HUB 属公开页。
  const [namerOpen, setNamerOpen] = useState(false);
  const cards = [{
    name: '九法合一解读',
    sub: '选择档案 · 跑九法或查看/修改解读（八字·紫微·西占·七政·奇门·五运六气等 9 法综合）',
    bar: 'var(--skin-accent)',
    ico: 'nine',
    ...
    name: '临时起卦',
    sub: '六爻 · 梅花 · 小六壬 · 灵签 等，用已有档案起卦',
    bar: 'var(--gold-500)',
    ico: 'orbit',
    to: () => onNavigate('divination')
  }, {
    // REQ-121：黄历择日 —— 与九法合一/生肖流年/临时起卦同层级独立入口（纯国学工具，不混入起卦列表）
    name: '择日',
    sub: '黄历择日 · 宜忌/冲煞/逐时时课（免费确定性）',
    bar: 'var(--skin-accent)',
    ico: 'calendar',
    to: () => onNavigate('almanac')
  }, {
    // REQ-124：太乙神数 —— 与九法合一/生肖流年/临时起卦同层级独立入口（纯国学大势工具，不混入起卦列表）
    name: '太乙神数',
    sub: '年/月/日/时四计七十二局 · 主客定算（免费确定性）',
    bar: 'var(--gold-500)',
    ico: 'taiyi',
    to: () => onNavigate('taiyi')
  }, {
    // REQ-125：皇极经世 —— 与九法合一/生肖流年/临时起卦同层级独立入口（纯国学大势工具，不混入起卦列表）
    name: '皇极经世',
    sub: '元会运世 · 值年/月经/旬纬/日卦/时经卦（免费确定性）',
    bar: 'var(--skin-accent)',
    ico: 'huangji',
    to: () => onNavigate('huangji')
  }];
  // REQ-100：同一排 3 键（九法配对 / 八字配对 / 档案起名），与上方三卡同层级同样式。
  const quickCards = [{
    name: '九法配对',
    sub: '双人合盘 · 缘分与相处建议（付费 LLM，按实际用量扣余额）',
    id: 'fn-pair',
    onClick: () => openPairAnalyze({ module: 'guoxue' })
  }, {
    name: '八字配对',
    sub: '双方八字排盘判相性（付费 LLM，按实际用量扣余额）',
    id: 'fn-bazi',
    onClick: () => openPairAnalyze({ module: 'bazi' })
  }, {
    name: '档案起名',
    sub: '依档案八字五行喜用推荐名字（付费 LLM，按实际用量扣余额）',
    id: 'fn-name',
    onClick: () => setNamerOpen(true)
  }];
  return /*#__PURE__*/React.createElement("div", {
    className: "container"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    name: "nine",
    size: 22
  }), UI_COPY.module_hub.guoxue_title), /*#__PURE__*/React.createElement("div", {
    className: "hub-grid"
    ...
    className: "hub-card",
    onClick: q.onClick
  }, /*#__PURE__*/React.createElement(SpriteIcon, {
    id: q.id,
    size: 28
  }), /*#__PURE__*/React.createElement("div", {
    className: "hc-body"
    ...
    className: "hc-sub"
  }, q.sub))))), namerOpen ? /*#__PURE__*/React.createElement(NamerModal, {
    caseId: null,
    caseName: "",
    onClose: function () {
      setNamerOpen(false);
...
function XishiHubPage({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    name: "orbit",
    size: 22
  }), UI_COPY.module_hub.western_title), /*#__PURE__*/React.createElement("div", {
    className: "hub-grid"
  }, cards.map((c, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: 'hub-card' + (c.disabled ? ' disabled' : ''),
    onClick: () => c.disabled ? toast('该模块敬请期待') : c.to()
  }, /*#__PURE__*/React.createElement("span", {
    className: "hc-bar",
    style: {
      background: c.bar
...
function MbtiHubPage({
  ...
  return React.createElement('div', {
    ...
    name: 'spark',
    size: 22
  }), UI_COPY.module_hub.mbti_title), React.createElement('div', {
    className: 'hub-grid'
...
function ZodiacRelModal({
  ...
  }, hd, sub, inner, disclaimer, hasMe ? el('div', {
    className: 'pair-foot'
  }, el('button', {
    type: 'button',
    className: 'btn btn-outline',
    onClick: onClose
  }, UI_COPY.buttons.close)) : null));
}
...
function DivLiurenView(res) {
  ...
  if (guaTi.length || pTags.length) {
    ...
    items.push(React.createElement('div', { key: 'gt-h', className: 'lr-sec' }, '课体'),
      React.createElement('div', { key: 'gt', className: 'lr-tags' },
        tags.map(function (t, i) { return React.createElement('span', { key: i, className: 'lr-tag' }, t); })));
  ...
  if (tjpKeys.length) {
    const tjpEls = tjpKeys.map(function (k) {
      const v = tjp[k] || {};
      const line = [v.wuxing ? v.wuxing : '', v.yinYang ? v.yinYang : '', v.category ? v.category : ''].filter(Boolean).join(' · ');
      return React.createElement('span', { key: k, className: 'lr-tag' },
        k + (line ? '：' + line : '') + (v.description ? '（' + v.description + '）' : ''));
    });
    items.push(React.createElement('div', { key: 'tj-h', className: 'lr-sec' }, '天将'),
      React.createElement('div', { key: 'tj', className: 'lr-tags' }, tjpEls));
  }
  ...
  if (ss.length) {
    items.push(React.createElement('div', { key: 'ss-h', className: 'lr-sec' }, '神煞'),
      React.createElement('div', { key: 'ss', className: 'lr-tags' },
        ss.map(function (s, i) { return React.createElement('span', { key: i, className: 'lr-tag' }, s); })),
...
function DivQimenView(res) {
  ...
  if (tags.length) {
    items.push(React.createElement('div', { key: 'pt-h', className: 'qm-sec' }, '格局（基础标签）'),
      React.createElement('div', { key: 'pt', className: 'qm-tags' },
        tags.map(function (t, i) { return React.createElement('span', { key: i, className: 'qm-tag' }, t); })),
...
function DivinationPage({
  ...
  return React.createElement('div', { className: 'container' },
    React.createElement('div', { className: 'hub-title' }, React.createElement(Icon, { name: 'orbit', size: 22 }), ' 临时起卦'),
    React.createElement('div', { className: 'card' },
      React.createElement('div', { className: 'section-title' }, '选择起卦方式'),
      React.createElement('div', { className: 'method-list' }, DIVIN_METHODS.map(function (x) {
        return React.createElement('div', {
          key: x.id,
...
function AlmanacPage({
  ...
  const addParticipant = function () {
    ...
    setParticipants(prev => prev.concat([{ name: '', gender: '男', year: '', month: '', day: '', timeIndex: 6 }]));
  };
  const patchPart = function (idx, patch) {
    setParticipants(prev => prev.map(function (p, i) { return i === idx ? Object.assign({}, p, patch) : p; }));
  };
  ...
  const dayCard = function (d) {
    ...
    return React.createElement('div', { key: d.date, className: 'alm-day-card ' + cls },
      React.createElement('div', { className: 'alm-day-head' },
  ...
  if (loading) {
    ...
    body = React.createElement('div', null,
      React.createElement('div', { className: 'card' },
        ...
        React.createElement('div', { className: 'alm-free' },
        ...
        React.createElement('button', { className: 'btn btn-outline', style: { marginTop: 8 }, disabled: interpLoading, onClick: interpret },
          interpLoading ? UI_COPY.tips.interpreting : [UI_COPY.buttons.interpret_pay, payBadge(payEnough)]),
        interpErr ? React.createElement('div', { className: 'error', style: { marginTop: 10 } }, interpErr) : null,
        interp ? React.createElement(React.Fragment, null,
    ...
    React.createElement('div', { className: 'alm-parts' }, participants.map(function (p, idx) {
      return React.createElement('div', { key: 'p' + idx, className: 'alm-part-item' },
        React.createElement('div', { className: 'alm-part-head' },
          React.createElement('span', { className: 'alm-part-title' }, '参与人 ' + (idx + 1)),
        ...
        React.createElement('div', { className: 'alm-part-grid' },
          React.createElement('div', { className: 'alm-part-field wide' },
            React.createElement('span', { className: 'alm-part-cap' }, A.part_name_ph),
            React.createElement('input', { className: 'alm-part-input', type: 'text', maxLength: 20, value: p.name, onChange: function (e) { patchPart(idx, { name: e.target.value }); } })),
          React.createElement('div', { className: 'alm-part-field' },
            React.createElement('span', { className: 'alm-part-cap' }, A.part_gender),
            React.createElement('select', { className: 'alm-part-input', value: p.gender, onChange: function (e) { patchPart(idx, { gender: e.target.value }); } },
              React.createElement('option', { value: '男' }, '男'),
              React.createElement('option', { value: '女' }, '女'))),
          React.createElement('div', { className: 'alm-part-field' },
            React.createElement('span', { className: 'alm-part-cap' }, '年'),
            React.createElement('input', { className: 'alm-part-input', type: 'number', min: 1900, max: 2100, placeholder: '1990', value: p.year, onChange: function (e) { patchPart(idx, { year: e.target.value }); } })),
          React.createElement('div', { className: 'alm-part-field' },
            React.createElement('span', { className: 'alm-part-cap' }, '月'),
            React.createElement('input', { className: 'alm-part-input', type: 'number', min: 1, max: 12, placeholder: '5', value: p.month, onChange: function (e) { patchPart(idx, { month: e.target.value }); } })),
          React.createElement('div', { className: 'alm-part-field' },
            React.createElement('span', { className: 'alm-part-cap' }, '日'),
            React.createElement('input', { className: 'alm-part-input', type: 'number', min: 1, max: 31, placeholder: '12', value: p.day, onChange: function (e) { patchPart(idx, { day: e.target.value }); } })),
          React.createElement('div', { className: 'alm-part-field wide' },
            React.createElement('span', { className: 'alm-part-cap' }, A.part_time),
            React.createElement('select', { className: 'alm-part-input', value: p.timeIndex, onChange: function (e) { patchPart(idx, { timeIndex: Number(e.target.value) }); } },
              ALMANAC_SHICHEN.map(function (s) {
                return React.createElement('option', { key: s.i, value: s.i }, s.label);
              })))));
    })),
    React.createElement('button', { type: 'button', className: 'alm-part-add', onClick: addParticipant }, '＋ ' + A.part_add),
    React.createElement('div', { className: 'alm-range-note' }, A.part_hint),
    React.createElement('div', { className: 'alm-run' },
      React.createElement('button', { className: 'btn btn-primary', type: 'button', disabled: loading, onClick: run },
        loading ? A.running : A.run),
      React.createElement('span', { className: 'alm-run-note' }, '确定性计算、零 LLM、免费；深度解读为可选付费（按实际用量扣余额）。')));
  return React.createElement('div', { className: 'container' },
...
function TaiyiPage({
  ...
  if (loading) {
    ...
    kids.push(React.createElement('div', { key: 'free', className: 'alm-free' },
      React.createElement('div', { className: 'af-title' }, A.free_reading_title),
      React.createElement('div', { className: 'af-txt' }, freeTxt)));
    kids.push(React.createElement('div', { key: 'payhint', className: 'pay-hint' }, A.pay_hint));
  ...
  const formCard = React.createElement('div', { className: 'card' },
    ...
    React.createElement('div', { className: 'ty-run' },
      React.createElement('button', { className: 'btn btn-primary', type: 'button', disabled: loading, onClick: run },
        loading ? A.running : A.run),
      React.createElement('span', { className: 'ty-run-note' }, '确定性计算、零 LLM、免费；深度解读为可选付费（按实际用量扣余额）。')));
  return React.createElement('div', { className: 'container' },
...
function TarotPage({
  ...
  // REQ-061：塔罗抽牌动效阶段机（deal=出牌动画 → down=背朝上待翻 → done=全翻开=v2 布局）
  const [dealStage, setDealStage] = useState('done');
  const [flipped, setFlipped] = useState({});
  const [bulkFlip, setBulkFlip] = useState(false); // 一键全翻的 stagger 模式
  ...
  const draw = async sp => {
    ...
    try {
      ...
      const d = (res && res.data) || res;
      setData(d);
      // REQ-061：取数先行 → 动画开（且⑥图开）进入出牌演出；关=直接以 v2 正面布局出结果
      if (tcShouldPlayAnim() && tcShouldShowCards()) setDealStage('deal'); else setDealStage('done');
    } catch (e) {
  ...
  const meanItems = cards.map(function (c, i) {
    ...
    return React.createElement('div', { key: 'mean' + i, className: 'mean-item' },
  ...
  // REQ-061：塔罗 3D 翻牌 —— 单张点击翻转 / 「一键全翻」stagger；正面/逆位沿用 normTarotCard 的 img/reversed
  const flipOne = function (i) {
    if (dealStage !== 'down' || flipped[i]) return;
    const next = Object.assign({}, flipped);
    next[i] = true;
    setFlipped(next);
    const left = cards.filter(function (c, k) { return !next[k]; }).length;
    if (!left) setTimeout(function () { setDealStage('done'); }, 780); // 全部单翻 → 落回 v2 布局
  };
  const flipAll = function () {
    if (dealStage !== 'down') return;
    const left = cards.filter(function (c, i) { return !flipped[i]; }).length;
    if (!left) { setDealStage('done'); return; }
    const all = {};
    cards.forEach(function (c, i) { all[i] = true; });
    setBulkFlip(true);
    setFlipped(all);
    setTimeout(function () { setBulkFlip(false); setDealStage('done'); }, 700 + cards.length * 110);
  };
  const flipCardCol = function (c, i, interactive) {
    const n = normTarotCard(c);
    const flippedNow = !!flipped[i];
    const front = React.createElement(ArtCardFront, { img: n.img, name: n.name, suit: n.suit, rev: n.reversed, phColor: (TAROT_SUIT_COLOR[n.suit] || '#5B4B8A') });
    const card = React.createElement('div', {
      className: 'flip-card' + (flippedNow ? ' flipped' : '') + (interactive ? '' : ' lock'),
      style: { '--fdi': i },
      onClick: interactive ? function () { flipOne(i); } : undefined
    }, React.createElement('div', { className: 'fc-inner' }, React.createElement(ArtCardBack, { kind: 'taro' }), front));
    const kwLine = (n.keywords && n.keywords.length) ? n.keywords.join(' · ') : '';
    const kids = [];
    if (n.pos) kids.push(React.createElement('div', { key: 'pos', className: 'tc-pos' }, n.pos));
    kids.push(card);
    kids.push(React.createElement('div', { key: 'rv', className: 'fc-reveal' },
      React.createElement('div', { className: 'fc-name' }, n.name, n.reversed ? '（逆位）' : ''),
      kwLine ? React.createElement('div', { className: 'fc-kw' }, kwLine) : null));
    return React.createElement('div', { key: 'fc' + i, className: 'fan-col' + (flippedNow ? ' flipped' : '') }, kids);
  };
  const flipMeanItem = function (c, i) {
    const n = normTarotCard(c);
    const revealed = !!flipped[i];
    const kwLine = (n.keywords && n.keywords.length) ? n.keywords.join(' / ') : '';
    const meanText = n.reversed ? (n.reversedText || '') : (n.upright || '');
    const kids = [
      React.createElement('div', { key: 'head', className: 'mi-head' },
        React.createElement('span', { className: 'mi-pos' }, n.pos || ('第' + (i + 1) + '张')),
        React.createElement('span', { className: 'mi-name' }, n.name),
        React.createElement('span', { className: 'mi-orient' + (n.reversed ? ' rev' : ' up') }, n.reversed ? '逆位' : '正位')),
      React.createElement('div', { key: 'lock', className: 'mi-lock' }, '该牌尚未翻开 · 牌义待揭示')];
    if (meanText) kids.push(React.createElement('div', { key: 'txt', className: 'mi-txt' }, meanText));
    if (kwLine) kids.push(React.createElement('div', { key: 'kw', className: 'mi-kw' }, '关键词：', kwLine));
    return React.createElement('div', { key: 'mean' + i, className: 'mean-item' + (revealed ? ' revealed' : ' locked') }, kids);
  };
  let drawBody = null;
  if (loading) {
    ...
    if (cardImagesOn) {
      if (dealStage === 'deal') {
        // D1 出牌动画：牌堆 → N 张牌背依次飞向牌位（数据已在手，翻牌阶段揭示）
        const cols = cards.map(function (c, i) { return flipCardCol(c, i, false); });
        bodyKids.push(React.createElement(DealFly, { key: 'deal', cols: cols, onDone: function () { setDealStage('down'); } }));
      } else if (dealStage === 'down') {
        // D2/D3/D4：背朝上待翻 → 点牌背 3D 翻转 → 一键全翻（解析随翻牌逐条放行）
        const cols = cards.map(function (c, i) { return flipCardCol(c, i, true); });
        const flippedN = Object.keys(flipped).filter(function (k) { return flipped[k]; }).length;
        const left = cards.length - flippedN;
        const fan = React.createElement('div', { key: 'fan', className: 'fan-zone' + (bulkFlip ? ' bulk' : '') },
          React.createElement('div', { className: 'card-fan' }, cols));
        const ctrl = React.createElement('div', { key: 'ctrl', className: 'flip-ctrl' },
          React.createElement('span', { className: 'fc-progress' },
            '已翻开 ', React.createElement('b', null, flippedN), '/', cards.length,
            left ? ' 张 · 点击牌背翻牌，或一键全翻' : ' 张 · 已全部翻开'),
          React.createElement('button', { className: 'btn btn-primary flip-all-btn', disabled: !left, onClick: flipAll }, left ? UI_COPY.buttons.flip_all : UI_COPY.buttons.flip_done));
        const mp = React.createElement('div', { key: 'mp', className: 'mean-panel' },
          React.createElement('div', { className: 'mp-title' }, '各张牌义（随翻牌逐张揭示）'),
          cards.map(function (c, i) { return flipMeanItem(c, i); }));
        bodyKids.push(fan, mp, ctrl);
      } else {
        // done（=v2 布局）：全正面平铺 + 集中牌义段落（与 REQ-042 v2 现状一致）
        bodyKids.push(React.createElement('div', { key: 'fan', className: 'card-fan' }, cards.map(function (c, i) {
          return React.createElement(TarotCard, { key: i, item: normTarotCard(c), showMeaning: false });
        })));
      ...
      // REQ-066⑥ 关：纯文字列表模式（无任何卡面/牌背/扇形渲染）
      bodyKids.push(React.createElement('div', { key: 'mp', className: 'mean-panel' },
        React.createElement('div', { className: 'mp-title' }, '抽到的牌（纯文字模式）'),
        meanItems));
    }
  ...
  const menuBody = React.createElement('div', { className: 'card' },
    ...
    React.createElement('div', { className: 'method-list' }, TAROT_SPREADS.map(function (s) {
      return React.createElement('div', {
        key: s.id,
        className: 'method-cell',
        onClick: function () { if (mode === 'manual') openManualPick(s); else draw(s); }
  ...
  return React.createElement('div', { className: 'container' },
    ...
    step === 'menu' ? menuBody : (step === 'pick' ? pickBody : drawFoot),
...
function LenormandPage({
  ...
  // REQ-061：雷诺曼抽牌动效阶段机（deal=出牌动画 → down=背朝上待翻 → done=全翻开=v2 布局；雷诺曼无正逆位）
  const [dealStage, setDealStage] = useState('done');
  const [flipped, setFlipped] = useState({});
  const [bulkFlip, setBulkFlip] = useState(false);
  ...
  // REQ-061：雷诺曼 3D 翻牌（同构，无正逆位）—— 单张点击翻转 / 「一键全翻」stagger
  const flipOne = function (i) {
    if (dealStage !== 'down' || flipped[i]) return;
    const next = Object.assign({}, flipped);
    next[i] = true;
    setFlipped(next);
    const left = cards.filter(function (c, k) { return !next[k]; }).length;
    if (!left) setTimeout(function () { setDealStage('done'); }, 780);
  };
  const flipAll = function () {
    if (dealStage !== 'down') return;
    const left = cards.filter(function (c, i) { return !flipped[i]; }).length;
  ...
  const flipCardCol = function (c, i, interactive) {
    const n = lnCardOf(c);
    const flippedNow = !!flipped[i];
    const front = React.createElement(ArtCardFront, { img: n.img, name: n.name, suit: n.suit, phColor: (LN_SUIT_COLOR[n.suit] || '#3E7C6B') });
    const card = React.createElement('div', {
      className: 'flip-card' + (flippedNow ? ' flipped' : '') + (interactive ? '' : ' lock'),
      style: { '--fdi': i },
      onClick: interactive ? function () { flipOne(i); } : undefined
    }, React.createElement('div', { className: 'fc-inner' }, React.createElement(ArtCardBack, { kind: 'leno' }), front));
    const kwLine = (n.keywords && n.keywords.length) ? n.keywords.join(' · ') : '';
    const kids = [];
    if (n.pos) kids.push(React.createElement('div', { key: 'pos', className: 'tc-pos' }, n.pos));
    kids.push(card);
    kids.push(React.createElement('div', { key: 'rv', className: 'fc-reveal' },
      React.createElement('div', { className: 'fc-name' }, n.name),
      kwLine ? React.createElement('div', { className: 'fc-kw' }, kwLine) : null));
    return React.createElement('div', { key: 'fc' + i, className: 'fan-col' + (flippedNow ? ' flipped' : '') }, kids);
  };
  const flipMeanItem = function (c, i) {
    ...
    const kids = [
      React.createElement('div', { key: 'head', className: 'mi-head' },
        ...
        React.createElement('span', { className: 'mi-name' }, n.name)),
      React.createElement('div', { key: 'lock', className: 'mi-lock' }, '该牌尚未翻开 · 牌义待揭示')];
    if (n.theme) kids.push(React.createElement('div', { key: 'th', className: 'mi-theme' }, n.theme));
    if (n.detail) kids.push(React.createElement('div', { key: 'dt', className: 'mi-txt' }, n.detail));
    if (kwLine) kids.push(React.createElement('div', { key: 'kw', className: 'mi-kw' }, '关键词：', kwLine));
    return React.createElement('div', { key: 'mean' + i, className: 'mean-item' + (revealed ? ' revealed' : ' locked') }, kids);
  };
  let body = null;
...
function SunLoveModal({
  ...
  }, hd, sub, inner, disclaimer, hasSun ? el('div', {
    className: 'pair-foot'
  }, el('button', {
    type: 'button',
    className: 'btn btn-outline',
...
function AstrologyPage({
  ...
  if (caseLoading) {
    ...
    // 档案选择卡：档案名 + meta「出生年月日 + 太阳星座(若有)」+ 圆形选中勾
    // （meta 不再显示性别/出生地；太阳星座仅当该档案已生成过星盘时附上）
    casePick = el('div', {
      className: 'case-opts'
    }, caseList.map(c => {
      const sel = caseId != null && Number(caseId) === Number(c.caseId);
      ...
      const meta = [birthCn, sunTxt].filter(Boolean).join(' · ') || '出生信息未补充';
      const nm = c.name || '档案 ' + c.caseId;
      return el('button', {
        key: c.caseId,
        type: 'button',
  ...
  // ---- REQ-115：三卡横向堆叠（覆盖 REQ-045 罗列三格呈现口径）——左右滑动切换当前卡；
  //      仅当前卡显示完整信息，其后两张卡 3/4 被当前卡遮挡 + 蒙版模糊；每卡星座名称前加
  //      I1 星座图标（cx-*，引用 icon-sprite-i1-i4.svg）。趋势文案内容（A1 72 篇）不变。 ----
  const tripleCurMax = trendCells.length - 1;
  const [tripleCur, setTripleCur] = useState(0);
  const [tripleDx, setTripleDx] = useState(0); // 拖动中横向位移（px）
  const [tripleDrag, setTripleDrag] = useState(false);
  const tripleStart = useRef({ x: 0, cur: 0 });
  const tripleMoved = useRef(false);
  const tripleDown = e => {
    tripleStart.current = { x: e.clientX, cur: tripleCur };
    tripleMoved.current = false;
    setTripleDrag(true);
  };
  const tripleMove = e => {
    if (!tripleDrag) return;
    const d = e.clientX - tripleStart.current.x;
    if (Math.abs(d) > 8) tripleMoved.current = true;
    setTripleDx(d);
  };
  const tripleEnd = () => {
    if (!tripleDrag) return;
    ...
    if (Math.abs(d) > 48) {
      const dir = d < 0 ? 1 : -1;
      const next = Math.max(0, Math.min(tripleCurMax, tripleStart.current.cur + dir));
      if (next !== tripleStart.current.cur) setTripleCur(next);
    }
  };
  const tripleCard = el('div', {
    ...
    style: {
      cursor: tripleDrag ? 'grabbing' : 'grab'
    },
    onPointerDown: tripleDown,
    onPointerMove: tripleMove,
    onPointerUp: tripleEnd,
    onPointerCancel: tripleEnd,
    onPointerLeave: tripleEnd,
    role: 'group',
    'aria-label': '太阳 / 月亮 / 上升 三张趋势卡，可左右滑动切换'
  }, trendCells.map((cell, i) => {
    const off = i - tripleCur;
    const isCur = off === 0;
    // 后卡偏移 %：右侧后卡约 3/4 被当前卡遮挡（仅露边缘）；更后一张再右移 8%；前卡整体隐藏于左侧
    const base = isCur ? 0 : (off < 0 ? -100 : off === 1 ? 72 : off === 2 ? 80 : -100);
    const follow = tripleDrag ? tripleDx * 0.45 : 0;
    const txt = zodiacTrendText(cell.key, cell.sign, trendGender);
    return el('div', {
      key: cell.key,
      className: 'triple-card' + (isCur ? '' : ' back'),
      style: {
        transform: 'translateX(' + base + '%) translateX(' + (isCur ? tripleDx : follow) + 'px)',
        transition: tripleDrag ? 'none' : 'transform .3s ease',
        zIndex: isCur ? 3 : (off > 0 ? 3 - off : 1)
      },
      onClick: isCur ? null : () => {
        if (!tripleMoved.current) setTripleCur(i);
      },
      'aria-label': (isCur ? '当前卡：' : '后卡：') + cell.title + ' · ' + (cell.sign || '未知')
    }, el('div', {
      className: 'tc-head'
    }, el('div', {
      className: 'tc-title'
      ...
      if (caseLoading) {
        ...
        gate = el(React.Fragment, null,
          el('div', {className: 'card'},
            ...
            el('div', {className: 'case-opts'},
              caseList.map(c => {
                const sel = caseId != null && Number(caseId) === Number(c.caseId);
                const meta = [c.birthYear ? c.birthYear + '年' : '', c.gender === 'male' ? '男' : c.gender === 'female' ? '女' : '', c.birthplace || ''].filter(Boolean).join(' · ') || '出生信息未补充';
                const nm = c.name || '档案 ' + c.caseId;
                const tval = caseTypeOf(c); // 已测类型 / ''=已确认未测 / null=探测中
                const typeNode = tval ? el('span', {className: 'co-type'}, '已测 ' + tval)
                  : (tval === null && typeLoading) ? el('span', {className: 'co-type none'}, '检测中…')
                  : el('span', {className: 'co-type none'}, '未测试');
                return el('button', {key: c.caseId, type: 'button', className: 'case-opt' + (sel ? ' sel' : ''), onClick: () => { setStarted(false); setCaseId(c.caseId); }},
                  el('span', {className: 'co-main'},
                    el('span', {className: 'co-name'}, nm),
                    typeNode,
                    el('span', {className: 'co-meta'}, meta)),
                  el('span', {className: 'co-check'}, sel ? '✓' : ''));
              }))),
...
function AgentPage({
  ...
  const switchModal = switchTarget ? el('div', {
    className: 'modal-mask',
    onClick: function () { if (!switchBusy) setSwitchTarget(null); }
  }, el('div', {
    className: 'modal',
    onClick: function (e) { e.stopPropagation(); }
  }, el('div', { className: 'modal-title' }, '切换档案'),
    ...
    el('p', { style: { fontSize: 13, color: 'var(--text-2)', lineHeight: 1.7, margin: '0 0 6px' } },
      '要将它设为默认档案，还是仅本次临时使用？'),
    el('div', { className: 'flex-row', style: { justifyContent: 'center', gap: 10, marginTop: 16, flexWrap: 'wrap' } },
      ...
      el('button', { type: 'button', className: 'btn btn-outline', disabled: switchBusy, onClick: function () { setSwitchTarget(null); } }, UI_COPY.buttons.cancel)))) : null;
...
// 九法合一页实际名 nine-pick → /jiufa；auth 页登录/注册同页（内部 tab 切换）→ 正向统一写 /login，
...
function App() {
  ...
  const [skin, setSkin] = useState(() => localStorage.getItem('taichu_skin') || 'guoxue');
  const [module, setModule] = useState(() => {
    const s = localStorage.getItem('taichu_skin') || 'guoxue';
  ...
  // tcSavedRef = 服务端最近确认值（用于保存失败回退）；tcLoadedRef = 本登录会话是否已拉取。
  const [tcSet, setTcSet] = useState(function () { return Object.assign({}, SETTINGS_DEFAULTS); });
  const [tcSave, setTcSave] = useState('saved'); // 'saved' | 'saving' | 'failed'
  const tcSavedRef = useRef(Object.assign({}, SETTINGS_DEFAULTS));
  const tcLoadedRef = useRef(false);
  const tcSeqRef = useRef(0); // PUT 序号：忽略乱序返回的旧响应，避免旧响应覆盖新改动
  ...
  const patchTcSettings = function (key, value) {
    setTcSet(function (prev) {
      const next = Object.assign({}, prev);
      next[key] = value;
      return next;
    });
  };
  const applyTcSettings = function (next) {
    tcSeqRef.current++; // 使任何在途 PUT 失效（GET/登出覆盖优先）
    tcSavedRef.current = next;
    setTcSet(next);
    setTcSave('saved');
  };
  // 拉取服务端设置：成功 → 覆盖；未登录 / 接口失败 → 前端默认值
  const loadTcSettings = async function () {
    try {
      const res = await api('/settings', { method: 'GET' });
      const raw = (res && res.data && res.data.settings) || (res && res.settings) || null;
      applyTcSettings(raw && typeof raw === 'object' ? normalizeTcSettings(raw) : Object.assign({}, SETTINGS_DEFAULTS));
    } catch (e) {
      applyTcSettings(Object.assign({}, SETTINGS_DEFAULTS));
    } finally {
  ...
  const doLogout = function () {
    logout();
    tcLoadedRef.current = false;
    applyTcSettings(Object.assign({}, SETTINGS_DEFAULTS));
  };
  // REQ-066：登录态 / 页面切换时拉取设置（启动带 token、登录/注册成功各拉一次；登出重置标记）
  useEffect(() => {
    if (token && page !== 'auth' && !tcLoadedRef.current) {
      loadTcSettings();
    } else if (!token) {
      tcLoadedRef.current = false;
    }
  ...
  // REQ-066：设置镜像 TC_SETTINGS（供各页面钩子读取）+ 防抖保存 —— tcSet 与「已确认值」
  // 不一致 → 450ms 后 PUT 全量设置；成功用服务端返回值覆盖（刷新确认基线），失败回退基线并标 failed。
  useEffect(() => {
    Object.assign(TC_SETTINGS, tcSet);
    if (shallowEqTc(tcSet, tcSavedRef.current)) return;
    setTcSave('saving');
    const seq = ++tcSeqRef.current;
    const timer = setTimeout(async () => {
      try {
        const res = await api('/settings', { method: 'PUT', body: JSON.stringify(tcSet) });
        if (seq !== tcSeqRef.current) return; // 已有更新的改动/覆盖，丢弃本次旧响应
        const raw = (res && res.data && res.data.settings) || (res && res.settings) || null;
        if (raw && typeof raw === 'object') {
          const next = normalizeTcSettings(raw);
          tcSavedRef.current = next;
          setTcSet(next);
        } else {
          tcSavedRef.current = Object.assign({}, tcSet);
        }
        setTcSave('saved');
      } catch (e) {
        if (seq !== tcSeqRef.current) return; // 旧请求失败不再回退（避免覆盖新改动）
        setTcSave('failed');
        setTcSet(Object.assign({}, tcSavedRef.current));
        if (e && (e.status === 401 || e.code === 401)) tcLoadedRef.current = false;
      }
    }, 450);
  ...
  useEffect(() => {
    ...
    // 入口 → 跳过本地 token 恢复：不进入「已登录自动进 cases」、不触发充值续查，内存会话保持空
    ...
    const saved = localStorage.getItem('taichu_token');
    if (saved) {
      token = saved;
      const savedR = localStorage.getItem('taichu_refresh_token');
      if (savedR) refreshToken = savedR;
      resumePendingRechargePoll(); // 从金数据充值表单整页跳转返回后，续查到账
    }
  ...
  const applySkin = s => {
    setSkin(s);
    localStorage.setItem('taichu_skin', s);
  };
  ...
  const pages = {
    landing: /*#__PURE__*/React.createElement(LandingPage, {
      ...
      loggedIn: !!token,
      /* REQ-116：Q 版动画跟随「太初先生」开关（agent_enabled 关 → 不渲染；与 banner「对坐谈心」同一开关源） */
      agentEnabled: tcSet.agent_enabled !== false,
      onNavigate: navigate
    }),
    ...
    settings: /*#__PURE__*/React.createElement(SettingsPage, {
      ...
      saveState: tcSave,
      loggedIn: !!token,
      onPatch: patchTcSettings,
      onNavigate: navigate
    }),
    // REQ-076：太初先生 Agent 会话页（右上角图标入口进入）
  ...
  tcSet.bg_enabled && /*#__PURE__*/React.createElement("div", {
  ...
  }, UI_COPY.brand.name, /*#__PURE__*/React.createElement("span", {
    className: "dot"
  }, "·")), /*#__PURE__*/React.createElement("span", {
    className: "tagline"
  }, UI_COPY.brand.tagline)), /* REQ-071 + REQ-066③：正中「模块」下拉（在「太初」与右上角菜单/汉堡 之间，
     用 flex 中列 + space-between 等距留白，不遮挡两侧）；是否展示由功能设置
     banner_dropdown 控制（默认关 → 不渲染；开 → 常驻展示） */
  tcSet.banner_dropdown && /*#__PURE__*/React.createElement(TopbarModNav, {
    onGo: goFeature
    ...
    /* REQ-076：太初先生 Agent 图标入口（agent_enabled 控制显隐） */
    ...
    style: tcSet.anim_enabled ? undefined : { animation: 'none' }
    ...
    if (this.state.error) {
      ...
      return React.createElement('div', { className: 'container', style: { padding: '40px 16px', textAlign: 'center' } },
        React.createElement('div', { className: 'card failed-box' },
          React.createElement('div', { className: 'section-title' }, '页面出错了'),
          ...
          React.createElement('div', { className: 'back-row', style: { justifyContent: 'center' } },
            React.createElement('button', { className: 'btn btn-primary', style: { width: 'auto' }, onClick: () => { window.location.assign('/'); } }, UI_COPY.buttons.back_home))));
    }

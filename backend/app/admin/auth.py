...
- 账号存于 mingli_ops.admin_users（与 C 端 users 分离）；
...
- 管理员令牌为 python-jose HS256 JWT，含 role 与 type=admin，与 C 端 access token
  互不通用；
- Agent 运维令牌（REQ-050）为 settings.agent_api_token（MINGLI_AGENT_TOKEN）静态
...
def require_agent(authorization: str = Header(default="")) -> dict:
    ...
    与 admin 账号密码 / admin JWT（type=admin）**完全独立**：
      - token 常量时间比对 settings.agent_api_token（MINGLI_AGENT_TOKEN）；
    ...
    if not settings.agent_api_token:
        raise BizError(ERR_UNAUTHORIZED, "Agent 通道未启用（未配置 MINGLI_AGENT_TOKEN）")
    if not authorization or not authorization.startswith("Bearer "):
        raise BizError(ERR_UNAUTHORIZED, "未登录")
    token = authorization.removeprefix("Bearer ").strip()

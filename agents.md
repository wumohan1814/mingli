...
| 部署 | Docker 单镜像双运行时 + Caddy（`taichu.xyz`）；`ops/deploy/` |
| 主分支 | `master` |
...
7. **不做 UI / 文字类生产**：UI 需求文档 / 文字需求文档由用户与对应 Agent 处理（美术素材表见 `docs/美术素材表-第三轮.md`，文案交付见 `docs/文案交付-第三轮/`）；但**交互 / 逻辑落地是我的职责**（素材组只产素材，接入代码由我负责）。

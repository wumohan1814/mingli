...
> ⚠️ **本表在 2026-09-14 用户第四条指令后已改写**：原表把「线上域名」列为"有意不改的例外"，
> 用户指出「域名应该是可配置项，而且一开源我的域名就暴露了」→ 域名**已改为配置项并全部移出仓库**。
> 下面「已解决」一栏记的是改后的状态。

| 残留 | 位置 | 处置 / 为什么 |
|---|---|---|
| `<我的域名>` / `bb3a.<我的域名>` | 原在 `Caddyfile`、`ops/deploy/README.md`、`docs/登录指南.md`、`docs/Code-Wiki.md`、`docs/架构设计.md`、`00_根/导航.md`、`backend/app/admin/router.py`、`docs/archify/*.json`、`40_节/待办/节122` | ✅ **已解决（用户 09-14 更正）**：域名改为**可配置项** `MINGLI_SITE_DOMAIN`（`Caddyfile` 用 `{$MINGLI_SITE_DOMAIN}`；值只在未入库的 `.env`）；`bb3a` 接入点抽成**可选附加站点**（`ops/deploy/caddy-extra.example` → `.gitignore` 覆盖的 `caddy-extra.caddy`）；仓库内所有真实域名 / 服务器公网 IP / 隧道 IP / Tailscale 主机名**全换占位符** |
| `（旧私钥文件名）` | `ops/deploy/README.md` | ✅ **已脱敏**：仓库里改按 `<你的私钥文件>` 占位（真实名只在用户自己的 `~/.ssh/config`）；改名的安全步骤保留在 `ops/deploy/README.md` |
| `taichu`（作为旧名） | `40_节/完成/节147-*.md`（本节）、`40_节/完成/节145-*.md`、节文件「用户原话」栏 | **保留**：改名对照文档必须同时出现旧名与新名，否则映射失真；原话栏是逐字历史 |
| `taichu-h5-prototype` | `00_根/导航.md`、`40_节/完成/节110` | **保留**：`_archive/前端-vite-ts-参考实现/` 里**归档文件的真实文件名**（归档冻结不改） |
| `design_library/taichu` | `40_节/完成/节145`、`99_状态/决策.md`、本节 | **保留**：`.design_library/` 是设计工具产物、**目录名冻结**，文档引用必须与磁盘一致 |
| `TC-XXXXXX` | `backend/app/credits/labels.py` 注释、`test_credits_labels.py`、`standards/03` | **保留**：节146 已 DROP 的充值码**序列号格式串**（不是项目代号）；§七-1 的检查模式也不含 `TC-` |
| `--tc-*` 令牌族（273 个） | `tokens.css` 等 | ⚠️ **用户先说「CSS 的令牌也一起改了吧」，核查后未动**：`--tc-*` 逐条注释全是**颜色角色**（反白 / 紫晶 / 塔罗宝剑 / 金箔 / 木色 / 舞台 / 大衍棋），是**主题色语义族**而非项目代号；用户给这条指令的理由（"确保没有代码层面的「太初」"）在 `--taichu-*` 改完后**已经达成**。改它会产出 `--ml-white` 这种失去含义的名字，且属设计令牌契约变更 → 结论与依据已回报用户并记进 `99_状态/决策.md` |
| 合规文书里的**真实运营者姓名 / 手机号 / 邮箱** | `docs/legal/命理用户协议.md`、`docs/legal/命理隐私政策.md`、`frontend/public/legal/*.html` | ⚠️ **未动，待用户拍板**：与域名同一类问题但更敏感。矛盾点——线上合规**要求**写真实主体，开源又**不该**暴露 → 需「模板化 + 运行时注入」类策略（建议抽 `MINGLI_OPERATOR_*` 配置）。已记进 `99_状态/决策.md` 待办行 |
...
**Agent 当时推荐（09-14 早）：保留 `<我的域名>` / `bb3a.<我的域名>` 不改，登记为具名例外。**

依据（节142 原文）：**「不保留独立的"官方托管运营服务"……开发者本人部署的开源服务不算"官方"，仅限本人自行使用，不对外提供服务。APK 模式 C 绑定的是"部署者自己的服务器"，无官方服务器可绑。」**
→ 既然这条线上实例只是**发布/内容载体**、**不对外提供服务**，域名就不是产品标识，而是**部署者的一项自填配置**。

- 改域名要：注册新域名 → 改 DNS（阿里云 HiChina）→ 服务器重签 Let's Encrypt → `bb3a`（OpenClaw 接入点）同步改，**有一个线上中断窗口**
- 当时算出的"不改的代价"只有一条：`grep taichu` 会剩 9 个文件里的域名串

> ### ⚠️ 用户当场纠正了这个推荐（09-14，第四条指令，**最终口径以这里为准**）
>
> 用户原话：「我认为这个域名应该是一个可配置项，只不过现在我把它配置成了 <我的域名>（原话带真实域名，按隐私要求以占位替代）。**如果我开源的话，我的域名是不是也会跟着这个项目暴露**」
>
> **Agent 上一段推荐漏掉的正是这一面**：只算了「改动成本」，没算「**开源即暴露**」—— 这个项目是**要开源**的（节142 Prosperity 许可），
> 把自己的域名、服务器公网 IP、内网 IP、SSH 私钥文件名留在仓库里，等于**开源那天全部公开**。
>
> **据此落地的方案（两边都满足：线上照用 + 仓库里不出现）**：
> 1. `Caddyfile` 主站地址改 `{$MINGLI_SITE_DOMAIN}`（env 注入；缺配置则 Caddy **启动失败**，宁可报错也不让占位域名去申请证书）
> 2. `bb3a` 接入点抽成**可选附加站点**：`ops/deploy/caddy-extra.example`（示例）→ `ops/deploy/caddy-extra.caddy`（本地，`.gitignore` 排除）→ 用 `MINGLI_CADDY_EXTRA` 挂载
> 3. 仓库内真实域名 / 服务器公网 IP / 隧道 IP / Tailscale 主机名 / SSH 私钥文件名**全部换占位符**
> 4. `.env.example` 只放占位（`mingli.example.com`）；**真实值只在未入库的 `.env`**
> 5. `.gitignore` 新增 `ops/deploy/caddy-extra.caddy` 与 `docs/未公开/`
>
> **通用原则（本节确立）**：凡是能定位到个人的东西（域名 / IP / 私钥名 / 真实姓名与联系方式）**一律不进仓库** —— 只出现在 `.env`、`.gitignore` 覆盖的本地文件、或 `docs/未公开/`。写进 `ops/deploy/README.md` 的「个人基础设施」表。
...
- **`--tc-*` 不是项目代号**：它是主题色语义族（反白/花色/金箔/木色/舞台…），**不要**因为它名字里有 `tc` 就再去改一遍（核查结论见 `99_状态/决策.md`）
- **改名的迁移顺序不可调换**：`备份 → GitHub 仓库改名 → 部署目录 → .env 键名（**含新增 MINGLI_SITE_DOMAIN**）→ DB 文件名与列名 → 起服务验证`。**`.env` 键名与 DB 文件名必须同一窗口内完成**，否则服务起不来；**漏了 `MINGLI_SITE_DOMAIN` 则 caddy 直接起不来**
...
- **哪里难** → ① **「新串含旧串」的自嵌套**：节145 第一轮的 `太初`→`命理太初` 会二次前缀（靠固定顺序 + 复检抓出 11 处）；第二轮用户把品牌更正为「命理」后，同一坑再犯一次（`命理太初`→`命理` 会让已写成「命理太初」的地方失去语义，实测 4 处文件名 + 若干叙述句需手工修）——**根因是"最终字符串"没在开工前一次问清**。节147 的 `taichu`→`mingli` 天然无自嵌套，**但多了一类反向坑**：**新名 `mingli` 与旧参考项目同名**，机械替换会把「参考项目 mingli」也变成「本项目 mingli」→ 靠**守卫式正则**（`(?<![-/\w])mingli(?![-_\w])`）+ **文件白名单** + **显式保护 3 个真实标识**（`high-confidence-mingli-skill-main`、`wumohan1814/mingli`、`mingli-*.json`）解决

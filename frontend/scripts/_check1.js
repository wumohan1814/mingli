...
function startRechargePoll(preBalance, startAt) {
  ...
  const tick = async () => {
    if (Date.now() - rechargePollStartAt > RECHARGE_MAX_WAIT_MS) {
      stopRechargePoll();
      toast(TC_COPY.ui.balance['recharge-processing']);
      return;
    }
...
async function api(path, options = {}) {
  ...
  const parseError = e => {
    ...
    let msg = TC_COPY.ui.error['request-failed'];
    const d = e && e.detail;
  ...
  if (r.status === 401 && !options.skip401) {
    ...
    throw new Error(TC_COPY.ui.error['login-expired']);
  }
  ...
  let errMsg;
  if (Array.isArray(rawDetail)) errMsg = rawDetail.map(x => x && x.msg || JSON.stringify(x)).join('；');else if (typeof rawDetail === 'string' && rawDetail) errMsg = rawDetail;else if (errBody && typeof errBody.message === 'string' && errBody.message) errMsg = errBody.message;else errMsg = fmtTpl(TC_COPY.ui.error['http-status'], { status: r.status });
  ...
  const MODS = {
    guoxue: {
      ...
      mascot: {
        ...
        sprite: { ready: false, src: '', cols: 5, rows: 2, frames: 10, duration: 3.4, width: 256 },
        interact: []
      }
    },
    xishi: {
      ...
      mascot: {
        ...
        sprite: { ready: false, src: '', cols: 5, rows: 2, frames: 10, duration: 3.6, width: 256 },
        interact: []
      }
    },
    mbti: {
      ...
      mascot: {
        ...
        sprite: { ready: false, src: '', cols: 5, rows: 2, frames: 10, duration: 3.2, width: 256 },
        interact: []
      }
    }
  };
...
const MOD_NAV_GROUPS = [{
  ...
  items: [{
    ...
    label: TC_COPY.ui.nav['item-almanac'],
    page: 'almanac',
    module: 'guoxue',
    skin: 'guoxue'
  }, {
    // REQ-124：太乙神数（免档案纯国学大势工具，顶栏下拉与 HUB 入口对齐）
    label: TC_COPY.ui.nav['item-taiyi'],
    page: 'taiyi',
    module: 'guoxue',
    skin: 'guoxue'
  }, {
    // REQ-125：皇极经世（免档案纯国学大势工具，顶栏下拉与 HUB 入口对齐）
    label: TC_COPY.ui.nav['item-huangji'],
    page: 'huangji',
    module: 'guoxue',
    skin: 'guoxue'
  }]
...
function GuoxueHubPage({
  ...
  const cards = [{
    name: TC_COPY.ui.hub['guoxue-nine-name'],
    sub: TC_COPY.ui.hub['guoxue-nine-sub'],
    bar: 'var(--skin-accent)',
    ico: 'nine',
    ...
    name: TC_COPY.ui.hub['guoxue-divination-name'],
    sub: TC_COPY.ui.hub['guoxue-divination-sub'],
    bar: 'var(--gold-500)',
    ico: 'orbit',
    to: () => onNavigate('divination')
  }, {
    // REQ-121：黄历择日 —— 与九法合一/生肖流年/临时起卦同层级独立入口（纯国学工具，不混入起卦列表）
    name: TC_COPY.ui.hub['guoxue-almanac-name'],
    sub: TC_COPY.ui.hub['guoxue-almanac-sub'],
    bar: 'var(--skin-accent)',
    ico: 'calendar',
    to: () => onNavigate('almanac')
  }, {
    // REQ-124：太乙神数 —— 与九法合一/生肖流年/临时起卦同层级独立入口（纯国学大势工具，不混入起卦列表）
    name: TC_COPY.ui.hub['guoxue-taiyi-name'],
    sub: TC_COPY.ui.hub['guoxue-taiyi-sub'],
    bar: 'var(--gold-500)',
    ico: 'taiyi',
    to: () => onNavigate('taiyi')
  }, {
    // REQ-125：皇极经世 —— 与九法合一/生肖流年/临时起卦同层级独立入口（纯国学大势工具，不混入起卦列表）
    name: TC_COPY.ui.hub['guoxue-huangji-name'],
    sub: TC_COPY.ui.hub['guoxue-huangji-sub'],
    bar: 'var(--skin-accent)',
    ico: 'huangji',
  ...
  const quickCards = [{
    name: TC_COPY.ui.hub['guoxue-pair-name'],
    sub: TC_COPY.ui.hub['guoxue-pair-sub'],
    id: 'fn-pair',
    onClick: () => openPairAnalyze({ module: 'guoxue' })
  }, {
    name: TC_COPY.ui.hub['guoxue-bazi-name'],
    sub: TC_COPY.ui.hub['guoxue-bazi-sub'],
    id: 'fn-bazi',
    onClick: () => openPairAnalyze({ module: 'bazi' })
  }, {
    name: TC_COPY.ui.hub['guoxue-namer-name'],
    sub: TC_COPY.ui.hub['guoxue-namer-sub'],
    id: 'fn-name',
  ...
  if (loading) {
    ...
    body = React.createElement('div', null,
      React.createElement('div', { className: 'card' },
        ...
        React.createElement(TCTermPanel, { key: 'tcpanel', method: 'almanac' }),
...
function TarotPickBoard(props) {
  ...
  // 阵位条自动滚动：保证当前待取格在可视区内（长牌阵如十二宫 / 年运也无需手滑）
  useEffect(function () {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const frameInner = c
      ? React.createElement('div', { className: 'tp-front' + (c.reversed ? ' reversed' : '') },
          c.img
            ? React.createElement('img', { src: c.img, alt: c.name, loading: 'lazy', decoding: 'async' })
            : React.createElement('div', { className: 'tp-ph', style: { background: (TAROT_SUIT_COLOR[c.suit] || '#5B4B8A') } }, c.name))
      : React.createElement('div', { className: 'tp-empty' });
    slotKids.push(React.createElement('div', {
      key: 's' + i,
      className: 'tp-slot' + (revealed ? ' filled' : '') + (i === n ? ' cur' : '')
    },
      React.createElement('div', { ref: function (el) { slotEls.current[i] = el; }, className: 'tp-frame' }, frameInner),
      React.createElement('div', { className: 'tp-pos' }, (i + 1) + '、' + ((props.pos && props.pos[i]) || ''))));
  }
  const fanBtns = order.map(function (tok) {
    ...
    }, React.createElement('i', { className: 'tp-back' }));
  });
  return React.createElement('div', { className: 'tp-board' },
    ...
    React.createElement('div', { className: 'tp-slots' }, slotKids),
    React.createElement('div', { className: 'tp-fan' + (closing ? ' closing' : '') },
      React.createElement('div', { className: 'tp-fan-cap' }, '牌背朝下 · 左右滑动浏览 · 依直觉点选一张 · 选过即定不可反悔'),
      React.createElement('div', { className: 'tp-fanrow' }, fanBtns)));
}
...
function TarotPage({
  ...
  const drawFoot = React.createElement(React.Fragment, null,
    drawBody,
    inPickPlay ? null : React.createElement('div', { className: 'back-row fill' },
...
function App() {
  ...
  }, TC_COPY.ui.brand.name, /*#__PURE__*/React.createElement("span", {
    className: "dot"
  }, "·")), /*#__PURE__*/React.createElement("span", {
    className: "tagline"
  }, TC_COPY.ui.brand.tagline)), /* REQ-071 + REQ-066③：正中「模块」下拉（在「太初」与右上角菜单/汉堡 之间，
    ...
    if (this.state.error) {
      ...
      return React.createElement('div', { className: 'container', style: { padding: '40px 16px', textAlign: 'center' } },
        React.createElement('div', { className: 'card failed-box' },
          React.createElement('div', { className: 'section-title' }, TC_COPY.ui.error['page-error-title']),
          React.createElement('div', { className: 'error', style: { wordBreak: 'break-all', whiteSpace: 'pre-wrap' } },
            TC_COPY.ui.error['page-error-prefix'] + msg),
          React.createElement('div', { className: 'back-row', style: { justifyContent: 'center' } },
            React.createElement('button', { className: 'btn btn-primary', style: { width: 'auto' }, onClick: () => { window.location.assign('/'); } }, TC_COPY.ui.buttons.back_home))));
    }

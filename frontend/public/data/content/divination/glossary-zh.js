...
// 节131 另加 CONTENT.divination.shengbei（潮汕圣杯·掷筊 三态正文，独立于 glossary 组）
...
/* 节131：潮汕圣杯（掷筊）三态正文 —— 面向用户的正文唯一来源（界面标签在
   UI_COPY.divination['sb-*']；Node result 仅带机器口径的简短判读，不复制本正文）。
   三态口径为公版常识（圣杯=一平一凸=应允 / 笑杯=两平面=再问 / 阴杯=两凸面=不允），
   潮汕本地叫法差异与连掷规则待用户拍板/联网核实后再行补充。 */
CONTENT.divination.shengbei = {
  intro: '掷筊（掷杯）是潮汕、闽南与台湾民间「问神」的通行方式：心里默念所问之事，双手各执一枚筊杯掷下，以两杯落地时朝上的面定三态。',
  rule: '筊杯分两面：平面（阳面）与凸面（阴面）。两杯组合成三态——圣杯=一平一凸；笑杯=两平面；阴杯=两凸面。本产品一次一掷，结果只代表本次所问。',
  states: {
    sheng: {
      name: '圣杯',
      short: '应允',
      meaning: '一平一凸（一阴一阳），神示应允：所问之事可得允准，可按心意推进。',
      culture: '圣杯又称「圣筊」，是掷筊里最盼的结果，主应允、可行、心意可遂；民间常在重要决断前以此求一个「准」。'
    },
    xiao: {
      name: '笑杯',
      short: '再问',
      meaning: '两杯皆平面朝上，神示「再问」：所问未明或时机未到，宜把话说清、换种问法再掷一次。',
      culture: '笑杯又称「笑筊」，两平面相对似张口而笑，主未定、需再问；不算凶兆，只是这一问还没问清楚。'
    },
    yin: {
      name: '阴杯',
      short: '不允',
      meaning: '两杯皆凸面朝上，神示不允：所问之事当下不宜推进，宜暂缓或另作打算。',
      culture: '阴杯又称「哭杯」「怒杯」，两凸面朝上，主不允、不宜；民间遇此多先停下，回头斟酌所问与时机。'
    }
  },
  disclaimer: '以上为传统民俗文化的参考说明，不构成现实决策建议；请理性看待，重大决策请依据现实信息。'
};

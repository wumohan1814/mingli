# OpenClaw 接入自有域名(mingli.example.com)方案

日期:2026-09-06
状态:方案待 review(未执行)

## 1. 目标
复用海外域名 `mingli.example.com`,使杭州 ECS(<你的服务器 IP>,阿里云杭州,1.6G 内存)上的 OpenClaw 2026.9.2 可被**授权用户从任意网络配对**,同时**对陌生人不可见/不可连**。用户选择「**新加坡 ECS 绕行**」路径:公开项目与 OpenClaw 接入点都放在新加坡(海外),规避大陆 ICP 备案。
...
3. **证书(Let's Encrypt)**:`acme.sh` 签发 `x9f2.mingli.example.com`(海外正常签发);主推 **DNS-01**(需 DNS 服务商 API),或 **HTTP-01**(需 `80`)。
4. **nginx server block**:`listen 443 ssl; server_name x9f2.mingli.example.com;` 反代回杭州 OpenClaw,经 Tailscale(`proxy_pass http://<杭州 TS IP>:44733` 或 `https://<你的 Tailscale 主机名>`,执行时确认端口);透传 `Host` / `X-Forwarded-Proto` / WebSocket 升级头。

## 4. 最终架构
- `mingli.example.com` 主域 → 新加坡 ECS(公开项目,不动)
- OpenClaw 随机前缀子域(示例 `x9f2.mingli.example.com`,执行时生成) → DNS A 指向**新加坡 ECS**
...
2. 装 `acme.sh`,签 `x9f2.mingli.example.com`(DNS-01 / HTTP-01)

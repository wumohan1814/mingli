...
- **key 存哪**：**项目内不存 key** —— 由**系统环境变量** `MINGLI_LLM_API_KEY`（用户级）注入，项目目录内没有任何真实 key 文件。
  设置命令（一次性，**新开终端生效**）：Windows `setx MINGLI_LLM_API_KEY "<你的 key>"`；
  Linux `export MINGLI_LLM_API_KEY=...`（持久化写进 `~/.bashrc`，或服务器侧走 systemd `EnvironmentFile=` / 宿主机环境变量）。
- **读取优先级**：系统环境变量 > `.env` 文件（pydantic-settings 默认源顺序：`init > 环境变量 > dotenv > secrets > 默认值`），
  即 `.env` 里不写该键、由系统环境变量提供也行。
- **key 怎么读**：FastAPI 启动时 cwd 为 `backend`（`start.sh`/`start.bat` 均先 `cd backend`），
  pydantic-settings 以 `MINGLI_` 前缀 + `env_file=".env"` 读入 `Settings`（环境变量优先于 `.env`），最终落在
  `settings.llm_api_key` 单点。
- **零硬编码**：全项目 `.py` 源码 / `.env.example` / README / 文档 / `.env` 中均无真实 key 字面量；
  项目目录（含被 git 忽略的文件）内**零 key 文件**，真实 key 只存在于系统环境变量。
...
- **后台不再接受 key 写入（fail loud，2026-09-21 起）**：operator+ 运维端点 `POST /admin/ops/llm-key`
  保留路由与请求体形状不变，但**不再写 `backend/.env`**，一律返回明确错误（既有信封 `code=1003`
  ERR_CONFLICT，`detail` 给出 `setx` / `export` 指引）；被拒绝的尝试仍留一条
  `admin_audit_logs(action=change_llm_key)`。原因：环境变量优先级高于 `.env`，若继续写 `.env`
  会 ① 让项目内重新出现 key 文件 ② 环境变量已设时写了却不生效（静默失败）。
...
- 泄露即轮换：立即到 DeepSeek 平台作废旧 key、生成新 key 并**更新系统环境变量**（Windows 重新 `setx` 后新开终端；Linux 改 `export`/环境文件后重启服务）。
...
| 风险/缺口 | 说明 | 建议 |
| --- | --- | --- |
| 环境变量在用户注册表/系统环境中明文 | key 明文字符串存在 Windows 用户注册表（HKCU 环境变量）/ Linux 系统环境中，**同机其他进程（同用户上下文）可读**，无加密、无访问审计 | Phase2 引入 secret manager；开发机全盘加密；不在脚本 / 截图 / 日志 / 报错里回显该变量；多用户共享的机器上不要用系统级变量 |
| 无 key 轮换机制 | 当前只能手工重设系统环境变量（Windows `setx` 后新开终端 / Linux 改 `export` 后重启服务） | Phase2 做轮换 SOP / 半自动化 |
